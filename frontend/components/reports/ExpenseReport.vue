    ...
    <v-row class="mb-4 align-center">
      <v-col cols="12" md="6">
        <h1 class="text-h4 font-weight-bold color-primary">
          <v-icon icon="mdi-cash-minus" class="mr-2" color="error"></v-icon>
          Expense Report
        </h1>
    ...
    <v-card class="border rounded-xl" variant="flat">
      ...
      >
        ...
        <template v-slot:item.status="{ item }">
          <v-chip
            :color="item.status === 'Paid' ? 'success' : (item.status === 'Partially Paid' ? 'orange' : 'warning')"
            size="small"
...
const headers = [
  ...
  { title: 'Category / Sub-type', key: 'category', sortable: false },
  { title: 'Total Amount', key: 'amount', sortable: false, align: 'end' },
...
const exportCSV = () => {
  ...
  const rows = items.value.map(item => [
    ...
    `"${item.reference || ''}"`,
    `"${item.description || ''}"`,
    `"${item.SubType?.ParentType?.type_name || item.category || ''}"`,
